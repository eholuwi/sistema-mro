...
class TestRegraOficialFaixa20:
    def test_abaixo_do_minimo(self):
        assert "COMPRAR" in status(5, 10, 0)

    def test_igual_ao_minimo(self):
        assert "COMPRAR" in status(10, 10, 0)

    def test_zona_atencao_inferior(self):
        assert "ATENÇÃO" in status(11, 10, 0)

    def test_zona_atencao_limite(self):
        assert "ATENÇÃO" in status(12, 10, 0)  # 10 * 1.2

    def test_acima_da_faixa(self):
        assert "OK" in status(13, 10, 0)

    def test_minimo_zero_com_saldo(self):
        assert "OK" in status(5, 0, 0)

    def test_minimo_zero_sem_saldo(self):
        assert "COMPRAR" in status(0, 0, 0)

    def test_none_nao_quebra(self):
        assert "COMPRAR" in status(None, None, 0)

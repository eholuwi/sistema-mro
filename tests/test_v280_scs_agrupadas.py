    ...
    sugs = [
        {
            ...
            "qtd_sugerida": 50,
            "consumo_diario": 2.0,
            "comprar_ate": "2026-07-25",
            "part_number": "PN-A",
            "fornecedor_ultimo_preco": 3.0,
            "cc_sugerido": "21106 - MANUTENÇÃO",
        },
        {
            "categoria_sc": NAT_A,
            "prioridade_tier": 1,
            "prioridade": "🟠 Antecipar",
            "qtd_sugerida": 30,
            "consumo_diario": 1.0,
            "comprar_ate": "2026-07-19",
            "part_number": "PN-B",
            "fornecedor_ultimo_preco": None,
            "cc_sugerido": "21106 - MANUTENÇÃO",
        },
    ]
...
def test_criar_sc_agrupada_multi_item(db, make_item, registrar_consumo):
    ...
    grupo = next(g for g in P.gerar_scs_sugeridas(incluir_fornecedor=False) if g["label"] == "Spare Parts")
    ...
    with database.transaction() as c:
        row = c.execute("SELECT observacoes FROM solicitacoes_compra WHERE id=?", (sc_id,)).fetchone()

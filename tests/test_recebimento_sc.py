...
def sc_aberta(db, make_item):
    ...
    sc_id = conn.execute("SELECT id FROM solicitacoes_compra WHERE numero_sc='SC-001'").fetchone()["id"]
    conn.close()
...
def test_recebimento_total_fecha_sc(db, sc_aberta):
    item_id, sc_id, item_sc_id = sc_aberta
    ok, msg = F.registrar_recebimento_sc(
        sc_id, item_sc_id, 10, CC, "Alm", "Alm", "Forn", "2026-01-10", "NF-2"
    )
    assert ok, msg
...
def test_recebimento_excede_pendente_rejeita(db, sc_aberta):
    ...
    ok, msg = F.registrar_recebimento_sc(sc_id, item_sc_id, 999, CC, "Alm", "Alm", "Forn", "2026-01-10", "NF")
    assert ok is False
...
def test_atomicidade_rollback_em_falha(db, sc_aberta, monkeypatch):
    ...
    conn = db.get_connection()
    status_antes = conn.execute("SELECT status FROM solicitacoes_compra WHERE id=?", (sc_id,)).fetchone()[
        "status"
    ]
    conn.close()
    ...
    ok, msg = F.registrar_recebimento_sc(sc_id, item_sc_id, 4, CC, "Alm", "Alm", "Forn", "2026-01-10", "NF")
    ...
    conn = db.get_connection()
    n_mov = conn.execute("SELECT COUNT(*) AS n FROM movimentacoes WHERE item_id=?", (item_id,)).fetchone()[
        "n"
    ]
    status_depois = conn.execute("SELECT status FROM solicitacoes_compra WHERE id=?", (sc_id,)).fetchone()[
        "status"
    ]
    conn.close()

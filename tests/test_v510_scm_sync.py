...
def test_resolver_codigos_solicitantes(db, monkeypatch):
    ...
    with db.transaction() as conn:
        ...
        assert n == 1
        cod = conn.execute(
            "SELECT codigo FROM solicitantes_mro WHERE nome='Julyo Oliveira'").fetchone()[0]
    assert cod == "001053"
...
def test_sincronizar_popula_scs_itens_e_externos(db, make_item, monkeypatch):
    ...
    isc = conn.execute(
        "SELECT descricao_detalhada, quantidade_solicitada, origem FROM itens_sc "
        "WHERE sc_id=?", (sc["id"],)).fetchall()
    assert len(isc) == 1 and isc[0]["origem"] == "api_scm" and isc[0]["quantidade_solicitada"] == 10.0
    ext = conn.execute(
        "SELECT part_number, preco_unitario FROM itens_sc_externos WHERE sc_id=?", (sc["id"],)).fetchall()

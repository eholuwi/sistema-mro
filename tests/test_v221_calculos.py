...
def test_lead_time_sc7_nao_sobrescreve_cadastrado(db, make_item, tmp_path):
    ...
    linhas = [
        {"Filial": 1, "Tipo": 1, "Item": 1, "Pedido": "F1", "DT Emissao": "2026-05-01",
         "Produto": "PN-LT", "Descricao": "x", "Unidade": "UN", "Moeda": 1, "Quantidade": 10,
         "Prc Unitario": 5.0, "Vlr.Total": 50, "Qtd.Entregue": 10, "Saldo": 0,
         "Observacoes": "SC: 1", "Dt. Entrega": "2026-05-11"},  # delta 10
        {"Filial": 1, "Tipo": 1, "Item": 1, "Pedido": "F2", "DT Emissao": "2026-05-01",
         "Produto": "PN-LT", "Descricao": "x", "Unidade": "UN", "Moeda": 1, "Quantidade": 10,
         "Prc Unitario": 5.0, "Vlr.Total": 50, "Qtd.Entregue": 10, "Saldo": 0,
         "Observacoes": "SC: 2", "Dt. Entrega": "2026-05-21"},  # delta 20
    ]
...
def test_lead_time_sc7_ignora_delta_zero(db, make_item, tmp_path):
    ...
    linhas = [
        {"Filial": 1, "Item": 1, "Pedido": "F1", "DT Emissao": "2026-05-01", "Produto": "PN-LT0",
         "Moeda": 1, "Quantidade": 1, "Prc Unitario": 5.0, "Qtd.Entregue": 1,
         "Observacoes": "SC: 1", "Dt. Entrega": "2026-05-01"},  # delta 0 → ignorado
    ]
    path = str(tmp_path / "rel.xlsx")
    with pd.ExcelWriter(path, engine="openpyxl") as w:
...
def test_lead_time_recebimento_nao_sobrescreve(db, make_item):
    ...
    ok, _ = F.criar_sc("SC-LTR", "2026-01-01",
                       [{"item_id": item_id, "part_number": "PN-LTR", "nome_item": "i",
...
def test_giro_via_snapshots(db, make_item):
    ...
    with database.transaction() as c:
        for d, est in [(_dias_atras(60)[:10], 100), (_dias_atras(30)[:10], 50), (_dias_atras(1)[:10], 10)]:
            c.execute("INSERT INTO estoque_snapshots (item_id,data,estoque_atual,valor_estoque) VALUES (?,?,?,0)",
                      (item_id, d, est))

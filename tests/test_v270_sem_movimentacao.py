...
def test_saldo_inicial_e_ajuste_nao_contam_como_movimentacao(db, make_item):
    ...
    F.registrar_movimentacao(
        iid, "saida", 5, "21106 - MANUTENÇÃO", "Joao", "Joao", observacao="Ajuste - retirado sem requisição"
    )
...
def test_reposicao_inclui_sem_movimentacao_com_toggle(db, make_item):
    ...
    fila = P.gerar_sugestoes_reposicao(incluir_fornecedor=False, incluir_sem_movimentacao=True)
...
def test_parada_de_linha_sem_giro_sai_do_comprar_mas_revisavel(db, make_item):
    ...
    assert not any(s["part_number"] == "PN-PL" for s in P.gerar_sugestoes_reposicao(incluir_fornecedor=False))

...
def test_unidades_da_ui_aceitas_pelo_schema(db, un):
    ok, msg = F.salvar_item(
        f"PN-{un}", "x", "", un, "Importante", "Spare Parts", "Improdutivo", "ARM-01", "", 1, 1, 7
    )
    assert ok, msg
...
def test_importancias_da_ui_aceitas(db, imp):
    ok, msg = F.salvar_item(
        f"PN-{imp}", "x", "", "UN", imp, "Spare Parts", "Improdutivo", "ARM-01", "", 1, 1, 7
    )
    assert ok, msg
...
def test_tipos_da_ui_aceitos(db, tp):
    ok, msg = F.salvar_item(f"PN-{tp}", "x", "", "UN", "Importante", tp, "Improdutivo", "ARM-01", "", 1, 1, 7)
    assert ok, msg

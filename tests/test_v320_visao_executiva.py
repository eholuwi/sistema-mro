...
def test_composicao_por_tipo_agrupa_e_junta_outros():
    itens = [{"tipo_material": "Consumivel", "valor": 100},
...
def test_montar_visao_executiva_estrutura(db, make_item, registrar_consumo):
    ...
    assert vm["ano"] == ANO
    assert set(vm) == {"ano", "kpis", "valor_detalhe", "series", "abc",
                       "composicao_tipo", "rankings", "destaques"}
    k = vm["kpis"]
    assert k["valor_consumido_ytd"] == 80.0        # 20 un × R$4
    assert k["itens_consumidos_ytd"] == 1
    assert k["n_requisicoes_ytd"] == 2
    assert set(vm["rankings"]) == {"top_valor_consumido", "top_qtd_consumida",
                                   "top_valor_imobilizado", "top_dead_stock",
                                   "top_centro_custo", "top_emitente", "top_setor"}

...
def test_receber_todos_itens_pendentes_fecha_sc(db, make_item):
    ...
    sc_id = _abrir_sc("SC-REC-1", [
        {"item_id": a, "part_number": "PN-A", "nome_item": "A",
         "quantidade_solicitada": 4, "quantidade_pedido": 4},
        {"item_id": b, "part_number": "PN-B", "nome_item": "B",
         "quantidade_solicitada": 6, "quantidade_pedido": 6},
    ])
    pend = [it for it in F.listar_itens_sc(sc_id) if (it.get("pendente") or 0) > 0]
    assert len(pend) == 2
    for it in pend:  # replica o loop da UI "Receber por SC"
        ok, msg = F.registrar_recebimento_sc(sc_id, it["id"], it["pendente"], CC,
                                             "Alm", "Alm", "Forn", "2026-01-10", "NF-1")
        assert ok, msg
...
def test_recebimento_parcial_mantem_sc_aberta(db, make_item):
    ...
    sc_id = _abrir_sc("SC-REC-2", [
        {"item_id": a, "part_number": "PN-C", "nome_item": "C",
         "quantidade_solicitada": 10, "quantidade_pedido": 10},
    ])
    it = F.listar_itens_sc(sc_id)[0]
    ok, msg = F.registrar_recebimento_sc(sc_id, it["id"], 4, CC,
                                         "Alm", "Alm", "Forn", "2026-01-10", "NF-1")
    assert ok, msg

...
def test_valor_consumido_estimativa(db, make_item):
    ...
    F.registrar_movimentacao(
        item_id, "saida", 100, CC, "x", "x", data_hora=_dias_atras(200)
    )  # fora da janela 90d
...
def test_abc_valor_classes_a_b_c(db, make_item, registrar_consumo):
    ...
    a = make_item("PN-A", estoque=1000, minimo=1)
    _set_preco_ref(a, 1.0)
    b = make_item("PN-B", estoque=1000, minimo=1)
    _set_preco_ref(b, 1.0)
    c = make_item("PN-C", estoque=1000, minimo=1)
    _set_preco_ref(c, 1.0)
...
def test_abc_valor_ignora_ajuste_fisico(db, make_item, registrar_consumo):
    ...
    a = make_item("PN-AJ", estoque=100000, minimo=1)
    _set_preco_ref(a, 100.0)
...
def test_evolucao_valor_imobilizado(db, make_item):
    ...
    with database.transaction() as c:
        for d, val in [(_dias_atras(3)[:10], 100.0), (_dias_atras(1)[:10], 150.0)]:
            c.execute(
                """INSERT INTO estoque_snapshots (item_id,data,estoque_atual,valor_estoque)
                         VALUES (?,?,?,?)""",
                (item_id, d, 10, val),
            )
...
def test_export_colunas_financeiras(db, make_item):
    ...
    for col in ["Preço Ref", "Origem Preço", "Valor em Estoque", "Valor Consumido(YTD)", "Classe ABC(valor)"]:

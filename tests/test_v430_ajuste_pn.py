...
def test_registrar_movimentacao_grava_motivo(db, make_item):
    ...
    ok, _ = F.registrar_movimentacao(
        item_id=item_id, tipo="saida", quantidade=3,
        centro_custo=None, solicitante="ana", emitente="ana",
        observacao="AJUSTE: perda", motivo="Perda de Material",
    )
    assert ok
    conn = db.get_connection()
    row = conn.execute(
        "SELECT tipo, motivo, quantidade FROM movimentacoes "
        "WHERE item_id=? AND motivo IS NOT NULL", (item_id,)).fetchone()
    conn.close()
...
def test_registrar_movimentacao_sem_motivo_continua_valido(db, make_item):
    ...
    ok, _ = F.registrar_movimentacao(
        item_id=item_id, tipo="entrada", quantidade=10,
        centro_custo=None, solicitante="x", emitente="x",
    )
    assert ok
...
def test_categoria_movimentacao_rotulos():
    ...
    assert F.categoria_movimentacao({"motivo": "Perda de Material", "tipo": "saida", "quantidade": 2}) == "Perda de Material"
...
def test_mapa_pn_por_requisicao_indexa_pn(db, make_item):
    ...
    ok, _ = F.criar_requisicao(
        setor="MANUTENÇÃO", emitente="joao", centro_custo="21106 - MANUTENÇÃO",
        autorizador_tipo="Gestor", autorizador_nome="maria",
        entrega_individual=False, destinatarios=[], sesmt=False, sesmt_responsavel="",

...
def _add_fornecedor(codigo, nome_fantasia, email="", razao="", loja="1", telefone="", contato=""):
    with database.transaction() as c:
        c.execute(
            """INSERT INTO fornecedores
               ...
               VALUES (?,?,?,?,?,?,?,?,?,1,?)""",
            (codigo, loja, razao, nome_fantasia, "", email, telefone, contato, "", "2026-01-01"),
        )
...
def test_sc7_ingestao_grava_lead_time_dias(db, make_item):
        ...
        [
            {
                "Produto": "PN-LT1",
                "Pedido": "PO-1",
                "DT Emissao": "2026-01-01",
                "Dt. Entrega": "2026-01-08",  # delta = 7
                "Qtd.Entregue": 3,
                "Prc Unitario": 20.0,
                "Moeda": "BRL",
                "Observacoes": "SC: 123",
    ...
    with database.transaction() as c:
        row = c.execute(
            "SELECT lead_time_dias FROM precos_historico WHERE item_id=? AND origem='SC7'", (item_id,)
        ).fetchone()
        ...
        [
            {
                "Produto": "PN-LT2",
                "Pedido": "PO-7",
                "DT Emissao": "2026-02-01",
                "Dt. Entrega": "2026-02-11",  # delta = 10
                "Qtd.Entregue": 2,
                "Prc Unitario": 30.0,
                "Moeda": "BRL",
                "Observacoes": "",
...
_ITENS_BUSCA = [
    ...
    {
        "part_number": "10PP0008",
        "nome_item": "PAPEL OFICIO BRANCO A4",
